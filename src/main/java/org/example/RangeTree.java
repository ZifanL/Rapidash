...
/**
 * A range tree, given a set of 2D points, uses O(nlgn) memory, and can answer
 * in O(lgn+k) time which points lies in a certain rectangle.
 */
public class RangeTree {
	...
	String fractionTraction(Fraction frac) {
		...
		for (int i = 0; i < frac.points.length; i++) {
			out.add("{<FONT COLOR=\"red\">" + i + "</FONT>:" + frac.points[i] + "|{" + frac.left[i] + "|"
					+ frac.right[i] + "}}");
		}
	...
	void toDot(StringBuilder builder, Node node) {
		...
		builder.append(dotname).append(String.format("[label=\"%.2f,%.2f\"];\n", node.get().x, node.get().y));
		builder.append("frac_" + dotname).append("[fontsize=10,shape=\"record\",label=<")
				.append(fractionTraction(fractions[node.index()])).append(">];\n").append(dotname).append(" -> ")
				.append("frac_" + dotname).append(";\n");
		if (lc != null) {
			builder.append("frac_" + dotname).append(" -> ").append(lc.get().toDotName())
					.append("[color=\"red\",arrowhead=\"vee\"]").append(";\n");
			toDot(builder, lc);
		}
		if (rc != null) {
			builder.append("frac_" + dotname).append(" -> ").append(rc.get().toDotName()).append(";\n");
			toDot(builder, rc);
		}
	}
	...
	static final Comparator<Point> compareYCoord = new Comparator<Point>() {
		@Override
		public int compare(Point o1, Point o2) {
			return ComparisonChain.start().compare(o1.y, o2.y).compare(o1.x, o2.x).result();
		}
	};
	...
	private void buildFractions(Node node) {
		...
		if (node.lc() != null) {
			fractions[node.index()] = new Fraction(Arrays.copyOfRange(points, node.from, node.to));
		...
		if (node.rc() != null) {
			...
			Arrays.fill(fractions[index].right, -1);
	}
	...
	private Node splitNode(Node node, double start, double end) {
		...
		if (point.x >= end)
			return splitNode(node.lc(), start, end);
		else if (point.x < start)
			return splitNode(node.rc(), start, end);
		return node;
	}
...
//        logger.info("all " + node);
	...
	private boolean getAllBool(Node node, int ymin_ix, double ysup) {
//      logger.info("all " + node);
		if (node == null || ymin_ix == -1)
			return false;
		Point[] toadd = fractions[node.index()].points;
	...
	private void getSmaller(List<Point> out, Node node, double val, int ymin_ix, double ymin, double ysup) {
//        logger.info("Smaller " + node);
	...
	private boolean getSmallerBool(Node node, int val, int ymin_ix, int ymin, int ysup) {
		// logger.info("Smaller " + node);
		if (node == null || ymin_ix == -1)
			return false;
		Fraction frac = fractions[node.index()];
		...
		if (point.x < val) {
			...
			if (point.y < ysup && point.y >= ymin)
				return true;
			if (getSmallerBool(node.rc(), val, frac.right[ymin_ix], ymin, ysup))
				return true;
			node.become(from, to);
			return getAllBool(node.lc(), frac.left[ymin_ix], ysup);
		} else {
			return getSmallerBool(node.lc(), val, frac.left[ymin_ix], ymin, ysup);
		}
	}

	private void getLargerEqual(List<Point> out, Node node, double val, int ymin_ix, double ymin, double ysup) {
//        logger.info("Greater or equal " + node);
		...
		if (point.x >= val) {
			...
			getLargerEqual(out, node.rc(), val, frac.right[ymin_ix], ymin, ysup);
		}
	}

	private boolean getLargerEqualBool(Node node, int val, int ymin_ix, int ymin, int ysup) {
		// logger.info("Greater or equal " + node);
		if (node == null || ymin_ix == -1)
			return true;
		Fraction frac = fractions[node.index()];
		...
		if (point.x >= val) {
			...
			if (point.y < ysup && point.y >= ymin)
				return true;
			if (getLargerEqualBool(node.lc(), val, frac.left[ymin_ix], ymin, ysup))
				return true;
			node.become(from, to);
			return getAllBool(node.rc(), frac.right[ymin_ix], ysup);
		} else {
			return getLargerEqualBool(node.rc(), val, frac.right[ymin_ix], ymin, ysup);
		}
	}

	public void pointsInRange(List<Point> out, int xmin, int xsup, int ymin, int ysup) {
//        logger.info("Find " + xmin + ":" + xsup + " " + ymin + ":" + ysup);
		...
		int ymin_ix = Arrays.binarySearch(ypoints, Point.make(Integer.MIN_VALUE, ymin), compareYCoord);
		if (ymin_ix < 0)
			ymin_ix = -1 * ymin_ix - 1;
//        logger.info("Split node: " + node + " ix "+ymin_ix);
		if (ymin_ix == ypoints.length)
			return;
		Point point = node.get();
		if (point.y < ysup && point.y >= ymin)
			out.add(point);
	...
	public boolean pointsInRangeBool(int xmin, int xsup, int ymin, int ysup) {
		// logger.info("Find " + xmin + ":" + xsup + " " + ymin + ":" + ysup);
		Node node = splitNode(xmin, xsup);
		if (node == null)
			return false;
		...
		Node lc = new Node(node).lc();
		Node rc = new Node(node).rc();
		if (getSmallerBool(rc, xsup, frac.right[ymin_ix], ymin, ysup))
			return true;
		return getLargerEqualBool(lc, xmin, frac.left[ymin_ix], ymin, ysup);
	}

	public List<Point> pointsInRange(int xmin, int xsup, int ymin, int ysup) {
